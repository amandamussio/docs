...
<ParamField path="idContrato" type="string" required>
...
Configure o endpoint do webhook nas configurações da sua integração com a Wincred. Certifique-se de que sua URL está:

- Acessível via HTTPS
- Preparada para receber requisições POST
- Configurada para retornar status HTTP 200 em caso de sucesso

### Exemplo de Implementação

<CodeGroup>
```javascript Node.js
app.post('/webhook/wincred/contratos', async (req, res) => {
  const { idContrato, evento, status, dataCriacao } = req.body;

  try {
    // Processar o evento
    await processarEventoContrato({
      idContrato,
      status,
      dataCriacao
    });
...
@app.route('/webhook/wincred/contratos', methods=['POST'])
def webhook_contratos():
    ...
    try:
        # Processar o evento
        processar_evento_contrato(id_contrato, status, data_criacao)
...
try {
    // Processar o evento
    processarEventoContrato($idContrato, $status, $dataCriacao);
...
<CardGroup cols={2}>
  <Card title="Idempotência" icon="rotate">
    Implemente lógica idempotente para processar eventos duplicados. Use o `idContrato` + `status` como chave única.
  </Card>
  
  <Card title="Retry Automático" icon="arrows-rotate">
    A Wincred tentará reenviar o webhook em caso de falha. Retorne status 200 o mais rápido possível.
  </Card>
  
  <Card title="Processamento Assíncrono" icon="clock">
    Processe eventos pesados de forma assíncrona. Receba o webhook, retorne 200 e processe em background.
  </Card>
  
  <Card title="Logs e Monitoramento" icon="chart-line">
    Registre todos os eventos recebidos para auditoria e troubleshooting. Monitore falhas no processamento.
  </Card>
</CardGroup>

<Warning>
  **Importante:** Nunca exponha sua URL de webhook publicamente sem autenticação. Configure validação de assinatura ou tokens de segurança.
</Warning>
...
<CardGroup cols={2}>
  <Card title="Consultar Detalhes" icon="magnifying-glass" href="/api-reference/contratos/get">
    Consulte informações detalhadas sobre um contrato específico
  </Card>
  
  <Card title="Webhooks de Parcelas" icon="webhook" href="/webhooks/parcelas">
    Configure webhooks para acompanhar o pagamento de parcelas
  </Card>
</CardGroup>